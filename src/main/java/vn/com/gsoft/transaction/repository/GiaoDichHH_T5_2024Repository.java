...
import vn.com.gsoft.transaction.entity.GiaoDichHangHoa_T5_2024;
...
@Repository
public interface GiaoDichHH_T5_2024Repository extends CrudRepository<GiaoDichHangHoa_T5_2024, Long> {
    @Query(value = "SELECT TOP(:top) " +
            ...
            ", SUM(c.TongSoLuong) as Tong FROM GiaoDichHangHoa_T5_2024 c" +
            " WHERE 1=1 " +
    ...
    @Query(value = "SELECT TOP(:top) " +
            ...
            " FROM " +
            "(SELECT c.ThuocId,c.tenThuoc, c.tenNhomNganhHang, c.tenDonVi" +
            ", SUM(c.tongBan) as Tong FROM GiaoDichHangHoa_T5_2024 c" +
            " WHERE 1=1 " +
            " AND (:#{#param.fromDate} IS NULL OR c.NgayGiaoDich >= :#{#param.fromDate})" +
            " AND (:#{#param.toDate} IS NULL OR c.NgayGiaoDich <= :#{#param.toDate})" +
            " AND (:#{#param.nhomDuocLyId} IS NULL OR c.nhomDuocLyId = :#{#param.nhomDuocLyId}) "+
    ...
    @Query(value = "SELECT TOP(:top) " +
            ...
            " FROM " +
            "(SELECT c.ThuocId,c.tenThuoc, c.tenNhomNganhHang, c.tenDonVi" +
            ", MAX(c.TSLN) as Tong FROM GiaoDichHangHoa_T5_2024 c" +
            " WHERE 1=1 " +
            " AND (:#{#param.fromDate} IS NULL OR c.NgayGiaoDich >= :#{#param.fromDate})" +
            " AND (:#{#param.toDate} IS NULL OR c.NgayGiaoDich <= :#{#param.toDate})" +
            " AND (:#{#param.nhomDuocLyId} IS NULL OR c.nhomDuocLyId = :#{#param.nhomDuocLyId}) "+
    ...
    @Query(value =
            "(SELECT c FROM GiaoDichHangHoa_T5_2024 c" +
                    " WHERE 1=1 " +
                    " AND (:#{#param.fromDate} IS NULL OR c.NgayGiaoDich >= :#{#param.fromDate})" +
                    " AND (:#{#param.toDate} IS NULL OR c.NgayGiaoDich <= :#{#param.toDate})" +
                    " AND (:#{#param.nhomDuocLyId} IS NULL OR c.nhomDuocLyId = :#{#param.nhomDuocLyId}) "+
    ...
    List<GiaoDichHangHoa_T5_2024> searchList(@Param("param") GiaoDichHangHoaReq param);
}
