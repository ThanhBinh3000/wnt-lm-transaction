...
@Repository
public interface GiaoDichHH_T3_2024Repository extends CrudRepository<GiaoDichHangHoa_T3_2024, Long> {
    ...
    @Query(value = "SELECT TOP(:top) " +
            ...
            "FROM GiaoDichHangHoa_T3_2024 c" +
            " WHERE 1=1 " +
            " AND (:#{#param.fromDate} IS NULL OR c.NgayGiaoDich >= :#{#param.fromDate})" +
            " AND (:#{#param.toDate} IS NULL OR c.NgayGiaoDich <= :#{#param.toDate})" +
            " AND (:#{#param.nhomDuocLyId} IS NULL OR c.nhomDuocLyId = :#{#param.nhomDuocLyId}) "+
    ...
    List<GiaoDichHangHoa_T3_2024> searchList(@Param("param") GiaoDichHangHoaReq param);
}
